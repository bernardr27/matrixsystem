...
import { initSupabaseMiddleware } from '@matrix-lib/supabase/next-middleware';

export async function middleware(request: NextRequest) {
    const { supabase, getResponse } = initSupabaseMiddleware(request);

    const { getLoginRedirectUrl } = await import('@matrix-lib/supabase');
    const { data: { user } } = await supabase.auth.getUser();
    const allowGuest = process.env.ROCKET_ALLOW_GUEST === '1' || process.env.NODE_ENV !== 'production';
