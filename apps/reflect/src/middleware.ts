...
export async function middleware(request: NextRequest) {
    ...
    const allowGuest = process.env.REFLECT_ALLOW_GUEST === '1' || process.env.NODE_ENV !== 'production';

    // If unauthorized and navigating to a core route, repel to Citadel master gateway.
    if (!user && !allowGuest && !request.nextUrl.pathname.startsWith('/api') && !request.nextUrl.pathname.startsWith('/_next') && request.nextUrl.pathname !== '/manifest.json') {
